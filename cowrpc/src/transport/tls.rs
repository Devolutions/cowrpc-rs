...
impl TlsOptionsBuilder {
    ...
    pub fn cert_from_pkcs12(mut self, der: &[u8], password: &str) -> Self {
        self.pkcs12 = Some(Pkcs12{der: Vec::from(der), password: String::from(password)});
        self
    }
    ...
    pub fn acceptor(self) -> Result<TlsOptions> {
        ...
        let pkcs12 = match self_mut.pkcs12.take() {
            Some(i) => i,
            _ => return Err(CowRpcError::Internal("Missing tls identity, you need to set a certificate".to_string())),
        };
