// The code in this file is taken from the hyper project, under MIT/Apache license.
// https://github.com/hyperium/http/blob/master/src/uri
...
impl Uri {
    pub fn get_addrs(&self) -> Result<Vec<net::IpAddr>, UriError> {
        if let Some(host) = self.host() {
            let ips: Vec<net::IpAddr> = match lookup_host(host) {
                Ok(hosts) => hosts,
                Err(_) => return Err(UriError(ErrorKind::Empty)),
            };
    ...
    fn new(mut s: ByteStr) -> Result<Uri, UriError> {
        if s.len() == 0 {
            ...
            let slash = auth_end;
            if s.len() == slash || s.as_bytes()[slash] != b'/' {
                s.insert(slash, '/');
            }
    ...
    fn path_start(&self) -> usize {
        self.authority_end.unwrap_or_else(|| self.scheme_end.unwrap_or(0))
    }
    ...
    pub fn host(&self) -> Option<&str> {
        self.authority().map(|auth| {
            ...
            if host_port.as_bytes()[0] == b'[' {
                let i = host_port.find(']').expect("parsing should validate matching brackets");
                &host_port[1..i]
            } else {
...
impl From<http::Uri> for Uri {
    fn from(uri: http::Uri) -> Uri {
        uri.to_string().parse().expect("attempted to convert invalid uri")
    }
}
...
impl From<Uri> for http::Uri {
    fn from(uri: Uri) -> http::Uri {
        ...
        http::Uri::from_shared(bytes).expect("attempted to convert invalid uri")
    }
}
...
pub fn scheme_and_authority(uri: &Uri) -> Option<Uri> {
    if uri.scheme_end.is_some() {
        Some(Uri {
            source: uri
                .source
                .slice_to(uri.authority_end.expect("scheme without authority")),
